...
### SLAM++:
  ...
  The ICP algorithm used in SLAM++. 
...
### Appendix
  ...
  If $$ \boldsymbol{A} $$ has rank $$ r $$, i.e. there are $$ r $$ nonzero elements in the diagonal matrix $$ \boldsymbol{\Sigma} $$ ($$ \boldsymbol{\Sigma} = diag(\sigma_1, \dots, \sigma_r) $$), then:
