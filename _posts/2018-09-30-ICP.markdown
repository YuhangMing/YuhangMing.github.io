...
#### Solutions:
  ...
  - Linear Estimation
    ...
    The most popular linear estimation method is Singular Value Decomposition (SVD), which is discussed thoroughly in Appendix 1. Here we focus on how SVD can be used to solve the optimal transformation.
    ...
    - Reduce the number of parameters
      ...
      the least square term can now be expressed as below with mathematical details in Appendix 2:
    ...
    - Find $$ \boldsymbol{R}^* $$
      ...
      now we try to isolate $$\boldsymbol{R}$$ out of the summation (mathematical details can be found in Appendix 3):
      ...
      where \boldsymbol{\Sigma} is a diagonal matrix of sigular values in descending order, and \boldsymbol{U} and \boldsymbol{V} are both diagonal matrices. When matrix $$ \boldsymbol{H} $$ is full rank,

      | $$ \boldsymbol{R}^* = \boldsymbol{U}\boldsymbol{V}^T $$ |

      Mathematical details about the optimality can be found in Appendix 4.
...
#### Appendix
  ...
  \begin{split}
    ...
    &= \parallel (\boldsymbol{p_i} - \boldsymbol{p}  - \boldsymbol{R} ( \boldsymbol{p_i}' - \boldsymbol{p}' ) ) + (\boldsymbol{p} - \boldsymbol{R}\boldsymbol{p}' -\boldsymbol{t}) \parallel^2
  \end{split}
  $$
